...
if (isset($_POST['submit'])) {
    ...
    if (empty($firstname)) {
        ...
        $_SESSION['add-user'] = "Mật khẩu xác nhận phải có ít nhất 8 ký tự!";
    } elseif (empty($avatar['name'])) {
        $_SESSION['add-user'] = "Vui lòng tải lên ảnh đại diện!";
    } else {
        // kiểm tra mật khẩu xác nhận
        if ($createpassword !== $confirmpassword) {
            $_SESSION['add-user'] = "Mật khẩu xác nhận không đúng!";
        } else {
            // mã hóa password
            ...
            if (mysqli_num_rows($user_check_result) > 0) {
                ...
                if (in_array($ext, $allowed_files)) {
                    ...
                    if ($avatar['size'] < 1000000) {
                        ...
                        $_SESSION['add-user'] = "Kích thước ảnh không được lớn hơn 1MB!";
                    }
                } else {
                    $_SESSION['add-user'] = "File không phải là ảnh!";
                }
            }
        }
    }
    ...
    if (isset($_SESSION['add-user'])) {
        ...
        $_SESSION['add-user-data'] = $_POST; // lưu lại thông tin đã nhập
